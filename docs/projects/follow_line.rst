...
Thus, instead of having both motors running at 15cm/s,  we could use
`drivetrain.set_speed(15+correction, 15-correction)`.
...
farther off we are,  the more we need to turn. Line position is a number between 
0-100; thus, the desired (neutral) position is 50, so error is defined as `error = line_position - 50`.
...
.. code-block:: python
    ...
    Kp = 0.2
    while True:
        error = linearray.line_pos()-50
        drivetrain.set_speed(15+Kp*error, 15-Kp*error)
...
The value of the coefficient `Kp=0.2` was chosen so that when the line is
all the way to one side (error= -50), the motors will be given speeds 
15-10=5 and 15+10=25, which is a reasonable range of speeds.


You can test what happens if Kp=0.2 is replaced by another value. If the
...
line, function `line_pos()` will return some random number. Thus, we need to test for that. 


A natural idea would be to replace `while True` by `while not all sensors are on black`, 
and then stop the motors when we reach the end of the line:
